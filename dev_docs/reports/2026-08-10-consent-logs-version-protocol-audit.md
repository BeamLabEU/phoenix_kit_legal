> **Superseded — kept for its evidence, not its fix plan.** This audit was written
> and acted on (`lib/phoenix_kit_legal/migrations/consent_logs.ex`, branch
> `backup/local-work-2026-08-10`) independently of, and in parallel with, the
> `main`-branch investigation now at
> [`2026-08-10-module-migration-versioning.md`](2026-08-10-module-migration-versioning.md)
> and its follow-up,
> [`2026-08-10-consent-logs-extraction.md`](2026-08-10-consent-logs-extraction.md).
> `main` took a different path: it deleted the coordinator this report's "What the
> fix looks like" and "Resolution" sections describe patching (0.3.0), pinned
> `phoenix_kit_consent_logs` as core-owned, and later replaced it with a new
> module-owned chain (`lib/phoenix_kit_legal/migrations.ex`) that never drops the
> table at all. The F1–F9 findings and live-verification evidence below predate
> and independently corroborate that `main` history — in particular F2/F3, the
> armed `DROP TABLE ... CASCADE` on a table Core actually owns, matches what the
> other two reports call out about the PR that "armed the rollback". The fix this
> report shipped on its own branch never reached `main`; do not read the
> "Resolution" section as the current state of the code.
